...
const FeaturedPet = async () => {
  ...
  const res = await fetch(`http://localhost:8000/addPets`, {
    next: { revalidate: 3600 } // প্রতি ১ ঘণ্টায় ডেটা অটো-রিফ্রেশ হবে (অপশনাল)
  });
    ...
    <section className="max-w-7xl mx-auto px-6 sm:px-12 lg:px-16 py-20 bg-transparent">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {displayedPets.map((pet) => (
          <div
            key={pet._id}
            className="group relative rounded-[2.5rem] bg-foreground/[0.03] dark:bg-zinc-900/40 backdrop-blur-md border border-foreground/[0.08] p-5 flex flex-col justify-between transition-all duration-500 hover:-translate-y-2 hover:border-pink-500/20 hover:shadow-[0_20px_50px_rgba(236,72,153,0.05)]"
          >
            <div>
              ...
              <div className="px-1 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs text-foreground/80 font-bold">
                  <div className="bg-foreground/[0.03] dark:bg-zinc-800/30 p-3 rounded-2xl border border-foreground/[0.03]">
            ...
            <div className="mt-6 px-1">
              <Link href={`/pets/${pet._id}`} className="block w-full">
